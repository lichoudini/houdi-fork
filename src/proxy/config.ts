...
const EnvSchema = z.object({
  ...
  PROXY_PLANNER_TIMEOUT_MS: z.coerce.number().int().positive().max(300_000).default(45_000),
  PROXY_VERIFIER_TIMEOUT_MS: z.coerce.number().int().positive().max(300_000).default(25_000),
  PROXY_OBJECTIVE_MAX_MS: z.coerce.number().int().positive().max(900_000).default(180_000),
  PROXY_PROGRESS_HEARTBEAT_MS: z.coerce.number().int().positive().max(120_000).default(4_000),
  PROXY_DETERMINISTIC_ROUTING_THRESHOLD: z.coerce.number().min(0).max(1).default(0.84),
  PROXY_AGENTIC_STATUS_ENABLED: z.string().optional(),
  PROXY_OBJECTIVE_STATE_DB_PATH: z.string().trim().default("./runtime/proxy-objective-state.sqlite"),
  ...
  PROXY_AGENT_POLICY_FILE: z.string().trim().default("./runtime/proxy-agent-policy.json"),
  PROXY_APPROVAL_TTL_MS: z.coerce.number().int().positive().max(86_400_000).default(600_000),
  ...
  PROXY_INTENT_ABSTAIN_THRESHOLD: z.coerce.number().min(0).max(1).default(0.68),
  PROXY_INTENT_ROUTING_THRESHOLD: z.coerce.number().min(0).max(1).default(0.72),
  PROXY_INTENT_CRITIC_ENABLED: z.string().optional(),
  PROXY_INTENT_SHADOW_ENABLED: z.string().optional(),
  PROXY_INTENT_BIAS_ENABLED: z.string().optional(),
  PROXY_INTENT_BIAS_FILE: z.string().trim().default("./runtime/proxy-intent-bias.json"),
  PROXY_INTENT_TELEMETRY_ENABLED: z.string().optional(),
  PROXY_INTENT_TELEMETRY_FILE: z.string().trim().default("./runtime/proxy-intent-telemetry.jsonl"),
  PROXY_INTENT_SLO_WINDOW: z.coerce.number().int().positive().max(1000).default(50),
  PROXY_INTENT_SLO_MAX_FAILURE_RATE: z.coerce.number().min(0).max(1).default(0.45),
  PROXY_INTENT_SLO_MIN_SAMPLES: z.coerce.number().int().positive().max(500).default(8),
});
...
export const proxyConfig = {
  ...
  resetWorkspaceOnStart: parseBooleanFlag(env.PROXY_RESET_WORKSPACE_ON_START, false),
  ...
  plannerTimeoutMs: env.PROXY_PLANNER_TIMEOUT_MS,
  verifierTimeoutMs: env.PROXY_VERIFIER_TIMEOUT_MS,
  objectiveMaxMs: env.PROXY_OBJECTIVE_MAX_MS,
  progressHeartbeatMs: env.PROXY_PROGRESS_HEARTBEAT_MS,
  deterministicRoutingThreshold: env.PROXY_DETERMINISTIC_ROUTING_THRESHOLD,
  agenticStatusEnabled: parseBooleanFlag(env.PROXY_AGENTIC_STATUS_ENABLED, true),
  objectiveStateDbPath: path.resolve(process.cwd(), env.PROXY_OBJECTIVE_STATE_DB_PATH),
  ...
  agentPolicyFile: path.resolve(process.cwd(), env.PROXY_AGENT_POLICY_FILE),
  approvalTtlMs: env.PROXY_APPROVAL_TTL_MS,
  ...
  intentAbstainThreshold: env.PROXY_INTENT_ABSTAIN_THRESHOLD,
  intentRoutingThreshold: env.PROXY_INTENT_ROUTING_THRESHOLD,
  intentCriticEnabled: parseBooleanFlag(env.PROXY_INTENT_CRITIC_ENABLED, true),
  intentShadowEnabled: parseBooleanFlag(env.PROXY_INTENT_SHADOW_ENABLED, true),
  intentBiasEnabled: parseBooleanFlag(env.PROXY_INTENT_BIAS_ENABLED, true),
  intentBiasFile: path.resolve(process.cwd(), env.PROXY_INTENT_BIAS_FILE),
  intentTelemetryEnabled: parseBooleanFlag(env.PROXY_INTENT_TELEMETRY_ENABLED, true),
  intentTelemetryFile: path.resolve(process.cwd(), env.PROXY_INTENT_TELEMETRY_FILE),
  intentSloWindow: env.PROXY_INTENT_SLO_WINDOW,
  intentSloMaxFailureRate: env.PROXY_INTENT_SLO_MAX_FAILURE_RATE,
  intentSloMinSamples: env.PROXY_INTENT_SLO_MIN_SAMPLES,
};
